...
void ManualInput(string& pattern, int& length, int& pattern_l, int& pattern_q) {
    ...
    if (pattern.empty()) {
        ...
        // Проверка уникальности введённой подстроки
        while (!hasUniqueChars(pattern)) {
            cout << "Error: characters in subsequence must be unique. Please enter another subsequence: ";
            getline(cin, pattern);
        }
        pattern_l = pattern.length();
        if (pattern_l > length) {
            cout << "Subsequence length exceeds main sequence length. Increasing main length to " << pattern_l << ".\n";
            length = pattern_l;
        }
        if (pattern_l > ALPHABET_SIZE) {
